...
def extract_all_user_followers_ids(screen_name: str) -> str:
    ...
    """
    _api = tweepy_api.get_twitter_api(os.path.join(PATH_TO_APP_DATA, 'twitter_api_keys.json'))
...
if __name__ == '__main__':
    ...
    main_api = tweepy_api.get_twitter_api(os.path.join(PATH_TO_APP_DATA, 'twitter_api_keys.json'))
    if _screen_name == '' or not _screen_name:
        _screen_name = main_api.me()._json['screen_name']
    _target_user = main_api.get_user(screen_name=_screen_name)
